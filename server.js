...
function safeSession(session) {
  ...
  return {
    ...
    applicationStatus: session.applicationStatus || null,
    ...
    payment: session.payment
      ? {
          ...
          paymentId: session.payment.paymentId || null,
          orderNumber: session.payment.orderNumber || null,
          financialTransactionReference:
            session.payment.financialTransactionReference || null,
          providerReference: session.payment.providerReference || null,
...
function extractProcessedPayment(code) {
  const processed = code?.processedPaymentData;
  if (!processed || typeof processed !== "object") return null;

  return {
    paymentId: processed.paymentId || processed.id || null,
    orderNumber: processed.orderNumber || null,
    financialTransactionReference:
      processed.financialTransactionReference || null,
    providerReference:
      processed.providerReference ||
      processed.channelReference ||
      processed.channel?.reference ||
      processed.provider?.reference ||
      null,
  };
}

function paymentMatchesSession(payment, session) {
  if (!payment || !session?.payment) return false;
  ...
  return Boolean(
    payment.reference === session.payment.reference ||
      ...
      payment.channel?.reference === session.payment.providerReference
  );
}

function paymentHasExpectedAmount(payment, session) {
...
function paymentVerificationResult(payment, session) {
  if (!payment) return { verified: false, reason: "not_found" };
  if (payment.status !== "completed") {
    return { verified: false, reason: "not_completed", status: payment.status };
  }
  if (!paymentHasExpectedAmount(payment, session)) {
    return { verified: false, reason: "amount_mismatch" };
  }
  if (!paymentMatchesSession(payment, session)) {
    return { verified: false, reason: "application_mismatch" };
  }
...
app.post("/api/ussd/session/start", async (req, res) => {
  try {
    ...
    const session = {
      ...
      applicationStatus: "offer_selection",
      createdAt: now,
      updatedAt: now,
    };
...
app.post("/api/ussd/session/:sessionId/payment-code", async (req, res) => {
  try {
    ...
    const updated = await createPaymentCodeForSession(session.sessionId);

    return res.json({
      success: true,
      reused: updated.sessionId === session.sessionId &&
        updated.payment?.paymentCodeId === session.payment?.paymentCodeId,
      session: safeSession(updated),
...
app.post("/api/ussd", async (req, res) => {
  try {
    ...
    // First request: persist a temporary gateway session so the next USSD
    // request can actually reach the enter_id state.
    if (!session) {
      const sessionId = gatewaySessionId || randomUUID();
      const now = new Date().toISOString();
      session = {
        sessionId,
        phone,
        ...
        payment: null,
        verifiedPayment: null,
        applicationStatus: "identity_required",
        createdAt: now,
        updatedAt: now,
      };
    ...
    // Never allow a gateway session ID to be reused with another phone.
    if (session.phone !== phone) {
      const error = new Error("USSD session does not belong to this phone number.");
      error.status = 403;
      throw error;
    }
...
async function handleUssdInput(session, input, phone, res) {
  if (session.step === "enter_id") {
    ...
    return res.json({
      ...
      response: [
        ...
        "Select an available loan offer:",
        "",
        offerMenu(),
      ].join("\n"),
      step: "offer_selection",
  ...
  if (session.step === "enter_payment_reference") {
    ...
    const verification = paymentVerificationResult(payment, session);
    ...
    if (verification.verified) {
      const updated = await updateSession(session.sessionId, {
        ...
        payment: {
          ...
          financialTransactionReference:
            payment.financialTransactionReference || null,
          providerReference: payment.channel?.reference || null,
        },
      });
    ...
    let message = "The payment has not been verified for this application yet.";
    if (verification.reason === "not_completed") {
      message = `Payment status: ${verification.status}. Please wait and try again.`;
    } else if (verification.reason === "amount_mismatch") {
      message = "The payment amount or currency does not match this application.";
    } else if (verification.reason === "application_mismatch") {
      message = "This payment is valid, but it is not linked to this Swift Loan application.";
    }

    return res.json({
      sessionId: session.sessionId,
      continueSession: true,
      response: `${message}\n\nEnter the correct payment reference to continue.`,
      step: "enter_payment_reference",
    });
  }
...
async function refreshPaymentCodeSession(session) {
  ...
  const code = data.result;
  const status = code.status || "pending";
  const processed = extractProcessedPayment(code);
    ...
    payment: {
      ...
      ...(processed
        ? {
            ...
            financialTransactionReference:
              processed.financialTransactionReference,
  ...
  // Payment Code completion is a useful signal, but final loan verification
  // still requires the actual Payment object to pass all reconciliation checks.
  if (status === "completed" && processed?.paymentId) {
    const paymentData = await monimeFetch(
      `/v1/payments/${encodeURIComponent(processed.paymentId)}`,
      { method: "GET", headers: monimeHeaders() }
    );
  ...
  return updateSession(session.sessionId, patch);
}
